let trendingId = sessionStorage.getItem('trendingID');

const API = 'api_key=0a6e02f4a23c2bedfd64e1ca9ccf492a';
const Movie_Details_URL = `https://api.themoviedb.org/3/movie/${trendingId}?${API}&language=en-US`;
const Movie_Video = `https://api.themoviedb.org/3/movie/${trendingId}/videos?${API}&language=en-US`;
const Moive_Poster_URL = `https://image.tmdb.org/t/p/w500/`;

getTrendingDetails(Movie_Details_URL);

function getTrendingDetails(data){
    fetch(data)
    .then((res) => res.json())
    .then((datas) => {
...
getTrendingVideo(Movie_Video);

function getTrendingVideo(data){
    fetch(data)
    .then((res) => res.json())
    .then((datas) => {
